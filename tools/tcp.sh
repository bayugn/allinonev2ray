...
installbbr(){
	...
	if [[ "${release}" == "centos" ]]; then
		# if [[ ${version} = "6" ]]; then
			...
			# else
				# echo -e "${Error} Still using 32 bits, don’t see you again !" && exit 1
			# fi
...
#Install the bbr2 kernel and integrate it into the xanmod kernel
#2021.3.15 https://github.com/UJX6N/bbrplus-5.10 bbrplusnew
installbbrplusnew(){
...
*               hard    nofile          1000000">/etc/security/limits.conf
	...
	read -p "After you need to restart the VPS，System optimization configuration will take effect，Whether to restart now ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
