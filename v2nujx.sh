...
continue_prompt() {
  read -rp "Continue other operations (yes/no)? " choice
  case "${choice}" in
...
checkIP() {
  local realIP4="$(curl -s `curl -s --ipv4 https://v4.ident.me/` -m 5)"
  local realIP6="$(curl -s `curl -s --ipv6 https://v6.ident.me/` -m 5)"
...
show_links() {
  if [ -f "/usr/local/bin/v2ray" ]; then
    ...
    colorEcho ${BLUE} "VMess (New sharing format)"
    ...
    colorEcho ${BLUE} "VMess (Old sharing format)"
...
get_v2ray() {
  if [ ! -f "/usr/local/bin/v2ray" ]; then
    ...
    cd $(mktemp -d)
    wget -q --show-progress "${v2ray_link}" -O v2ray-core.zip
...
get_naiveproxy() {
  if [ ! -f "/usr/local/bin/naive" ]; then
    ...
    cd $(mktemp -d)
    wget -q --show-progress "${naive_link}" -O naive.tar.xz
...
get_caddy() {
  if [ ! -f "/usr/local/bin/caddy" ]; then
    ...
    ${sudoCmd} wget -q --show-progress "${caddy_link}" -O /usr/local/bin/caddy && $(which chmod) +x /usr/local/bin/caddy
    printf "Installed: %s\n" "/usr/local/bin/caddy"
  ...
  if [ -f "/usr/local/bin/v2ray" ]; then
    while true; do
      ...
      if checkIP "${V2_DOMAIN}"; then
        colorEcho ${GREEN} "The domain name ${V2_DOMAIN} is resolved correctly, and the certificate will be repaired soon"
        break
      else
        colorEcho ${RED} "The domain name ${V2_DOMAIN} is parsed incorrectly (yes: force to continue, no: re-enter, quit: leave)"
        read -rp "If you are sure that the domain name is resolved correctly, you can continue the repair operation. Forced to continue? (yes/no/quit) " forceConfirm
    ...
    colorEcho ${GREEN} "Certificate repair completed"
    show_links
  else
    colorEcho ${YELLOW} "Please install first V2Ray"
  fi
}
...
install_v2ray() {
  while true; do
    read -rp "Resolve to the domain name of this VPS: " V2_DOMAIN
    if checkIP "${V2_DOMAIN}"; then
      colorEcho ${GREEN} "domain name ${V2_DOMAIN} The analysis is correct, and the installation will start soon"
      break
    else
      colorEcho ${RED} "domain name ${V2_DOMAIN} Wrong parsing (yes: Forced to continue, no:re-enter, quit:go away)"
      read -rp "If you are sure that the domain name is resolved correctly, you can continue the installation operation. Forced to continue? (yes/no/quit) " forceConfirm
      case "${forceConfirm}" in
        [yY]|[yY][eE][sS] ) break ;;
        [qQ]|[qQ][uU][iI][tT] ) return 0 ;;
      esac
...
edit_cf_node() {
  ...
  printf "3. %s\n" "www.garmin.com"
  printf "4. %s\n" "amp.cloudflare.com"
  read -p "Enter the new CF node address [Leave it blank to use the existing value ${cf_node_current}]: " cf_node_new
  case "${cf_node_new}" in
  ...
  sleep 1
  printf "%s\n" "CF The node has been changed to ${cf_node_new}"
  show_links
  fi
}
...
show_menu() {
  ...
  echo "5) Update v2ray-core"
  echo "6) Update trojan-go"
  echo "7) Update naiveproxy"
  echo "----------Utilities----------"
  echo "8) VPS Toolbox (including BBR script)"
  echo "----------Uninstall script----------"
  echo "9) Uninstall script and all components"
  echo ""
}
